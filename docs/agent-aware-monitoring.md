...
ScopeWarden can work in two modes:

1. Manual mission mode: you run `scopewarden start "mission"`.
2. Agent-aware mode: ScopeWarden reads local agent context and suggests or attaches the mission.
...
Only `--apply` writes `.scopewarden/session.json`.
...
ScopeWarden recursively scans likely JSON, JSONL, text, markdown, chat, transcript, and rollout files under those roots.

Native and Ollama launch aliases that ScopeWarden recognizes:
...
| Need | Command |
|---|---|
| Continue immediately | `scopewarden start "mission"` |
| Inspect one agent | `scopewarden agents context --agent codex` |
| Override a path | Edit `scopewarden.yaml` |
...
ScopeWarden gives higher confidence when:
...
ScopeWarden refuses to attach very low-confidence missions. Auto-attach is off by default and should stay opt-in.

## What gets filtered

ScopeWarden ignores common non-mission text:
...
`scopewarden mcp` exposes JSON-style methods for compatible tools:
...
`scopewarden skills install` and `scopewarden plugins install` create project-local assets. They do not claim native marketplace installs or automatic Stop hooks.

## Product principle

ScopeWarden should make the best path easy without hiding uncertainty:
